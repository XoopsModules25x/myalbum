    ...
    default:
        ...
        if (!empty($cids)) {
            foreach ($cids as $index => $child) {
                $catpath .= "<a href='" . XOOPS_URL . '/modules/' . $GLOBALS['mydirname'] . '/viewcat.php?num='
                            . (int)$GLOBALS['myalbum_perpage'] . '&cid=' . $child->getVar('cid') . "' >"
                            . $child->getVar('title') . '</a> ' . ($index < count($cids) ? '>>' : '');
            }
        } else {
            $cat = $cat_handler->get($photo_obj->getVar('cid'));
            $catpath .= "<a href='" . XOOPS_URL . '/modules/' . $GLOBALS['mydirname'] . '/viewcat.php?num='
                        . (int)$GLOBALS['myalbum_perpage'] . '&cid=' . $cat->getVar('cid') . "' >"
                        . $cat->getVar('title') . '</a>';
        }
        ...
        if ($numrows > 1) {
            ...
            if ($ids[0] != $lid) {
                ...
                $photo_nav .= "<a href='photo.php?lid=" . $ids[$pos - 1] . "'><b>" . _ALBM_PREVIOUS
                              . '</b></a>&nbsp;&nbsp;';
            }
            ...
            if ($ids[$numrows - 1] != $lid) {
                $photo_nav .= "<a href='photo.php?lid=" . $ids[$pos + 1] . "'><b>" . _ALBM_NEXT
                              . '</b></a>&nbsp;&nbsp;';
        ...
        $GLOBALS['xoopsTpl']->assign('xoops_pagetitle',
                                     $photo_obj->getVar('title') . ' : ' . $cat->getVar('title') . ' : '
