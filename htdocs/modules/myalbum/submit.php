...
if (!empty($_POST['submit'])) {
    ...
    if ($_FILES[$field]['name'] == '') {
        ...
        if (isset($GLOBALS['myalbumModuleConfig']['myalbum_canresize']) && $GLOBALS['myalbumModuleConfig']['myalbum_canresize']) {
            $uploader = new MyXoopsMediaUploader($GLOBALS['photos_dir'], explode(
                '|',
                $GLOBALS['myalbumModuleConfig']['myalbum_allowedmime']
            ), $GLOBALS['myalbumModuleConfig']['myalbum_fsize'], null, null, explode(
                '|',
    ...
    if ($status) {
        ...
        if (is_object($cat)) {
            $notification_handler->triggerEvent(
                ...
                $cid,
                'new_photo',
                array('PHOTO_TITLE' => $title, 'CATEGORY_TITLE' => $cat->getVar('title'), 'PHOTO_URI' => $photo_obj->getURL())
            );
        }
    }
