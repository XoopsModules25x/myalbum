...
if (!empty($_POST['myalbum_import']) && !empty($_POST['cid'])) {
    ...
    // create category
    $GLOBALS['xoopsDB']->query("INSERT INTO " . $GLOBALS['xoopsDB']->prefix($table_cat) . "(pid, title, weight, imgurl) SELECT '0',title,weight,imgurl FROM $src_table_cat WHERE cid='$src_cid'") or die("DB error: INSERT cat table");
