...
if (!empty($_POST['myalbum_import']) && !empty($_POST['cid'])) {
    ...
    $rs = $GLOBALS['xoopsDB']->query('SELECT conf_name,conf_value FROM  ' . $GLOBALS['xoopsDB']->prefix('config')
                                     . " WHERE conf_modid='$src_mid'");
    ...
    // create category
    $GLOBALS['xoopsDB']->query('INSERT INTO ' . $GLOBALS['xoopsDB']->prefix($table_cat)
                               . "(pid, title, imgurl) SELECT '0',title,imgurl FROM $src_table_cat WHERE cid='$src_cid'")
    ...
    while (list($src_lid, $ext) = $GLOBALS['xoopsDB']->fetchRow($rs)) {
        ...
        if ($move_mode) {
            ...
            list($photos_dir, $thumbs_dir, $myalbum_mid, $table_photos, $table_text, $table_votedata, $saved_photos_dir,
                $saved_thumbs_dir, $saved_myalbum_mid, $saved_table_photos, $saved_table_text, $saved_table_votedata) =
                array(
                    $src_photos_dir,
                    $src_thumbs_dir,
                    $src_mid,
    ...
    if (!empty($_POST['imagemanager_import']) && !empty($_POST['imgcat_id'])) {
        ...
        // create category
        $crs =
            $GLOBALS['xoopsDB']->query("SELECT imgcat_name,imgcat_storetype FROM $src_table_cat WHERE imgcat_id='$src_cid'");
        list($imgcat_name, $imgcat_storetype) = $GLOBALS['xoopsDB']->fetchRow($crs);

        $GLOBALS['xoopsDB']->query('INSERT INTO ' . $GLOBALS['xoopsDB']->prefix($table_cat) . "SET pid=0,title='"
                                   . addslashes($imgcat_name) . "'")
        || exit('DB error: INSERT cat table');
        $cid = $GLOBALS['xoopsDB']->getInsertId();

        // INSERT loop
        $rs           =
            $GLOBALS['xoopsDB']->query("SELECT image_id,image_name,image_nicename,image_created,image_display FROM $src_table_photos WHERE imgcat_id='$src_cid'");
        $import_count = 0;
        while (list($image_id, $image_name, $image_nicename, $image_created, $image_display) =
            $GLOBALS['xoopsDB']->fetchRow($rs)) {
            $src_file = XOOPS_UPLOAD_PATH . "/$image_name";
            $ext      = substr(strrchr($image_name, '.'), 1);

            // photos table
            $sql = 'INSERT INTO  ' . $GLOBALS['xoopsDB']->prefix($table_photos) . "SET cid='$cid',title='"
                   . addslashes($image_nicename)
            ...
            if ($imgcat_storetype === 'db') {
                ...
                $brs = $GLOBALS['xoopsDB']->query('SELECT image_body FROM  ' . $GLOBALS['xoopsDB']->prefix('imagebody')
            ...
            $GLOBALS['xoopsDB']->query('UPDATE ' . $GLOBALS['xoopsDB']->prefix($table_photos)
                                       . "SET res_x='$width',res_y='$height' WHERE lid='$lid'");
