...
function myalbum_adminMenu ($page, $currentoption = 0)
{
    ...
    if (file_exists(XOOPS_ROOT_PATH . '/modules/'.$GLOBALS['myalbumModule']->getVar('dirname').'/language/' . $GLOBALS['xoopsConfig']['language'] . '/modinfo.php')) {
        ...
        require_once XOOPS_ROOT_PATH . '/modules/'.$GLOBALS['myalbumModule']->getVar('dirname').'/language/english/modinfo.php';
    }
    ...
    $adminObject->displayNavigation($page);
    echo "</div>";
}
*/
/*
...
{
    echo "<div align=\"center\"><a href=\"https://xoops.org\" target=\"_blank\"><img src=" . XOOPS_URL . '/' . $GLOBALS['myalbumModule']->getInfo('icons32') . '/xoopsmicrobutton.gif'.' '." alt='XOOPS' title='XOOPS'></a></div>";
    echo "<div class='center smallsmall italic pad5'><strong>" . $GLOBALS['myalbumModule']->getVar("name") . "</strong> is maintained by the <a class='tooltip' rel='external' href='https://xoops.org/' title='Visit XOOPS Community'>XOOPS Community</a> and <a class='tooltip' rel='external' href='http://www.chronolabs.coop/' title='Visit Chronolabs Co-op'>Chronolabs Co-op</a></div>";
}
*/
 ...
 * @return string
 */
function mysqli_get_sql_set($cols)
{
    $ret = '';
...
function myalbum_create_thumbs_by_netpbm($src_path, $node, $ext)
{
    ...
    // Check the path to binaries of imaging packages
    if ('' != trim($myalbum_netpbmpath) && DIRECTORY_SEPARATOR != mb_substr($myalbum_netpbmpath, -1)) {
...
function myalbum_modify_photo_by_imagick($src_path, $dst_path)
{
    ...
    // Check the path to binaries of imaging packages
    if ('' != trim($myalbum_imagickpath) && DIRECTORY_SEPARATOR != mb_substr($myalbum_imagickpath, -1)) {
...
function myalbum_update_photo($lid, $cid, $title, $desc, $valid = null, $ext = '', $x = '', $y = '')
{
    ...
    if (isset($valid)) {
        ...
        if (1 == $valid) {
            ...
            // Global Notification
            $notificationHandler->triggerEvent('global', 0, 'new_photo', ['PHOTO_TITLE' => $title, 'PHOTO_URI' => $photo->getURL()]);
