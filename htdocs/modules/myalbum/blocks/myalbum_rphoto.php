...
if (!defined('MYALBUM_BLOCK_RPHOTO_INCLUDED')) {
    ...
    function b_myalbum_rphoto_show($options)
    {
        ...
        // Get number of photo
        $result = $xoopsDB->query('SELECT count(lid) FROM ' . $xoopsDB->prefix($table_photos)
                                  . " WHERE status>0 AND $whr_cat AND $whr_ext");
        ...
        if ($numrows <= $photos_num) {
            $result =
                $xoopsDB->query('SELECT lid , cid , title , ext , res_x , res_y , submitter , status , date AS unixtime , hits , rating , votes , comments FROM '
                                . $xoopsDB->prefix($table_photos) . " WHERE status>0 AND $whr_cat AND $whr_ext");
        } else {
            $result   = $xoopsDB->query('SELECT lid FROM ' . $xoopsDB->prefix($table_photos)
                                        . " WHERE status>0 AND $whr_cat AND $whr_ext");
            $lids     = array();
            $sel_lids = array();
            ...
            $result =
                $xoopsDB->query('SELECT lid , cid , title , ext , res_x , res_y , submitter , status , date AS unixtime , hits , rating , votes , comments FROM '
                                . $xoopsDB->prefix($table_photos) . " WHERE status>0 AND lid IN ($whr_lid)");
        }
    ...
    function b_myalbum_rphoto_edit($options)
    {
        ...
        <input type='text' size='6' name='options[1]' value='$box_size' style='text-align:right;' />&nbsp;pixel "
                . _ALBM_TEXT_BLOCK_WIDTH_NOTES . '
        <br />
        ' . _ALBM_TEXT_DISP . "&nbsp;
        ...
        <input type='radio' name='options[4]' value='1' " . ($cat_limit_recursive ? "checked='checked'" : '') . '/>'
                . _YES . "
        <input type='radio' name='options[4]' value='0' " . ($cat_limit_recursive ? '' : "checked='checked'") . '/>'
                . _NO . '
        <br />
