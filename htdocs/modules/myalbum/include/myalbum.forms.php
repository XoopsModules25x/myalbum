...
function myalbum_admin_form_admission()
{
    ...
    $cat_select       =
        new XoopsFormLabel(_AM_TH_CATEGORIES, $GLOBALS['cattree']->makeSelBox('cid', 'title', '--', null, false));
...
{
    $irs            =
        $GLOBALS['xoopsDB']->query('SELECT c.imgcat_id,c.imgcat_name,c.imgcat_storetype,COUNT(i.image_id) AS imgcat_sum FROM '
                                   . $GLOBALS['xoopsDB']->prefix('imagecategory') . ' c NATURAL LEFT JOIN '
...
function myalbum_admin_form_import_myalbum()
{
    ...
    $mrs            = $GLOBALS['xoopsDB']->query('SELECT dirname FROM ' . $GLOBALS['xoopsDB']->prefix('modules')
                                                 . " WHERE dirname like 'myalbum%'");
    $frm            = '';
    while (list($src_dirname) = $GLOBALS['xoopsDB']->fetchRow($mrs)) {
        ...
        $myalbum_cat_options =
            myalbum_get_cat_options('title', 0, '--', '----', $GLOBALS['xoopsDB']->prefix("{$src_dirname}_cat"),
                                    $GLOBALS['xoopsDB']->prefix("{$src_dirname}_photos"));
        ...
        $frm .= '<p>
                ...
                <input type='submit' name='myalbum_import' value='" . _GO . "' onclick='return confirm(\""
                . _AM_MB_IMPORTCONFIRM . "\");' />
                </form>\n";
...
function myalbum_admin_form_import_imagemanager()
{
    ...
    if ($sysperm_handler->checkRight('system_admin', XOOPS_SYSTEM_IMAGE, $GLOBALS['xoopsUser']->getGroups())) {
        ...
        $irs            =
            $GLOBALS['xoopsDB']->query('SELECT c.imgcat_id,c.imgcat_name,COUNT(i.image_id) AS imgcat_sum FROM '
                                       . $GLOBALS['xoopsDB']->prefix('imagecategory') . ' c NATURAL LEFT JOIN '
                                       . $GLOBALS['xoopsDB']->prefix('image')
                                       . ' i GROUP BY c.imgcat_id ORDER BY c.imgcat_weight');
        $imgcat_options = '';
        ...
        $frm .= '<p>
                ...
                <input type='submit' name='imagemanager_import' value='" . _GO . "' onclick='return confirm(\""
                . _AM_MB_IMPORTCONFIRM . "\");' />
                </form>\n";
        $frm .= '<br /></p>';
    }
...
function myalbum_admin_form_display_edit($cat_array, $form_title, $action)
{
    ...
    // Title
    $form->addElement(new XoopsFormText(_AM_CAT_TH_TITLE, 'title', 30, 50, $GLOBALS['myts']->htmlSpecialChars($title)),
                      true);
    ...
    // Image URL
    $form->addElement(new XoopsFormText(_AM_CAT_TH_IMGURL, 'imgurl', 50, 150,
                                        $GLOBALS['myts']->htmlSpecialChars($imgurl)));

    // Parent Category
    $form->addElement(new XoopsFormLabel(_ALBM_PHOTOCAT,
                                         $GLOBALS['cattree']->makeSelBox('pid', 'title', '--', $pid, true)));
...
function myalbum_user_form_submit($caller = '', $photo, $lid)
{
    ...
    $size_label   = new XoopsFormLabel(_ALBM_MAXSIZE, $myalbum_fsize
                                                      . (empty($file_uploads_off) ? '' : ' &nbsp; <strong>"file_uploads" off</strong>'));
    $form->setExtra("enctype='multipart/form-data'");

    $title_text = new XoopsFormText(_ALBM_PHOTOTITLE, 'title', 50, 255, $photo['title']);

    $cat_select = new XoopsFormLabel(_ALBM_PHOTOCAT, $GLOBALS['cattree']->makeSelBox('cid', 'title', '--',
        (isset($_REQUEST['cid']) ? (int)$_REQUEST['cid'] : null), false));
    ...
    if (isset($myalbum_canrotate)) {
        if ($myalbum_canrotate) {
            ...
            $rotate_radio->addOption('rot90', "<img src='assets/images/icon_rotate90.gif' alt='" . _ALBM_RADIO_ROTATE90
                                              . "' title='" . _ALBM_RADIO_ROTATE90 . "' /> &nbsp; ");
            $rotate_radio->addOption('rot180',
                                     "<img src='assets/images/icon_rotate180.gif' alt='" . _ALBM_RADIO_ROTATE180
                                     . "' title='" . _ALBM_RADIO_ROTATE180 . "' /> &nbsp; ");
            $rotate_radio->addOption('rot270',
                                     "<img src='assets/images/icon_rotate270.gif' alt='" . _ALBM_RADIO_ROTATE270
                                     . "' title='" . _ALBM_RADIO_ROTATE270 . "' /> &nbsp; ");
        }
    }
