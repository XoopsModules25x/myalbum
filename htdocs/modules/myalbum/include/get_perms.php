...
if (is_object($GLOBALS['xoopsDB'])) {
    ...
    if (isset($GLOBALS['myalbum_mid'])) {
        ...
        $rs                      =
            $GLOBALS['xoopsDB']->query('SELECT gperm_itemid FROM ' . $GLOBALS['xoopsDB']->prefix('group_permission')
                                       . " WHERE gperm_modid='" . $GLOBALS['myalbum_mid']
                                       . "' AND gperm_name='myalbum_global' AND ($whr_groupid)");
